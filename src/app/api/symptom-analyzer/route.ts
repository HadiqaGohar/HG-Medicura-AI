...
const RATE_LIMIT = { maxRequests: 10, timeWindow: 60000 };
const requestCounts = new Map<string, { count: number; resetTime: number }>();
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!rateLimit.allowed) {
      return NextResponse.json(
        { error: 'Too many requests. Try again later.', retryAfter: Math.ceil(RATE_LIMIT.timeWindow / 1000) },
        {
          status: 429,
          headers: {
            'X-RateLimit-Limit': RATE_LIMIT.maxRequests.toString(),
    ...
    const validSymptoms = validateSymptoms(body.symptoms);
    if (!validSymptoms) {
      return NextResponse.json({ error: 'At least one valid symptom is required' }, { status: 400 });
    }
...
`;
    ...
    const response = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        contents: [{ parts: [{ text: prompt }] }],
        generationConfig: { temperature: 0.1, maxOutputTokens: 1024, topP: 0.8, topK: 40 },
        safetySettings: [
          { category: 'HARM_CATEGORY_HATE_SPEECH', threshold: 'BLOCK_MEDIUM_AND_ABOVE' },
          { category: 'HARM_CATEGORY_DANGEROUS_CONTENT', threshold: 'BLOCK_MEDIUM_AND_ABOVE' },
          { category: 'HARM_CATEGORY_SEXUALLY_EXPLICIT', threshold: 'BLOCK_MEDIUM_AND_ABOVE' },
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('Gemini API error:', response.status, errorText);
      if (response.status === 429) return NextResponse.json({ error: 'Service busy. Try again later.' }, { status: 429 });
      if (response.status === 401 || response.status === 403) return NextResponse.json({ error: 'Authentication error' }, { status: 500 });
      return NextResponse.json({ error: 'Failed to analyze symptoms' }, { status: 502 });
    }
...
export async function GET() {
  return NextResponse.json(
    { status: 'OK', message: 'Symptom Analyzer API is running', timestamp: new Date().toISOString(), version: '1.0.0' },
    { status: 200 }
  );
}
